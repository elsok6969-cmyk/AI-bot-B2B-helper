...
> 4. Установит все зависимости (включая FastAPI/Telethon/aioimaplib), накатит миграции
> 5. Сгенерирует `SECRETS_KEY` (Fernet) сам, если в `.env` он пуст
> 6. Запустит бота, дождётся, пока веб-морда поднимется, и откроет её в браузере (<http://localhost:8090>)
>
> **Перед первым запуском** отредактируй `.env` и укажи `BOT_TOKEN`, `OWNER_TELEGRAM_ID`, API-ключ для AI. Опционально — `TELETHON_API_ID`/`TELETHON_API_HASH` с <https://my.telegram.org>, если нужен Telethon.
>
